...
- **Success Criteria**: MVP-COMM-001 through MVP-COMM-008 implemented and tested;
  ...
  collection; pre-delivered schema seeded on `cross.agency.created`; all
...
- **Schema seeding**: triggered on `cross.agency.created` event AND on startup if `CODEVALDCOMM_AGENCY_ID` env var is set (idempotent via `SetSchema`).
...
| MVP-COMM-007 | CodeValdCross Registration | 📋 Not Started | MVP-COMM-005, MVP-COMM-006 | `internal/registrar/registrar.go` — wraps `sharedlib/registrar`; implements `CrossPublisher`; subscribes to `cross.agency.created` for schema seeding; heartbeat every 20 s; `cmd/main.go` fully wired: ArangoDB connect → DataManager/SchemaManager → EntityServer + CommServer → HTTP handler → registrar → `serverutil.RunWithGracefulShutdown`; env vars: `CODEVALDCOMM_GRPC_PORT` (required), `COMM_ARANGO_ENDPOINT`, `COMM_ARANGO_USER`, `COMM_ARANGO_PASSWORD`, `COMM_ARANGO_DATABASE`, `CROSS_GRPC_ADDR`, `COMM_GRPC_ADVERTISE_ADDR`, `CODEVALDCOMM_AGENCY_ID`, `CROSS_PING_INTERVAL`, `CROSS_PING_TIMEOUT`; Cross registration: produces `cross.comm.{agencyID}.message.sent/edited/thread.promoted/reaction.added/member.joined/participant.presence`, consumes `cross.agency.created`; routes via `schemaroutes.RoutesFromSchema(DefaultCommSchema(), "/{agencyId}/comm", ...)` |
...
- [ ] Pre-delivered schema seeded into `comm_schemas` on `cross.agency.created` (idempotent)
- [ ] All HTTP convenience routes work end-to-end and are registered with CodeValdCross
- [ ] `cross.comm.{agencyID}.message.sent` published on every successful SendMessage
- [ ] `cross.comm.{agencyID}.message.edited` published on every successful EditMessage
- [ ] `cross.comm.{agencyID}.thread.promoted` published on every successful PromoteToThread
- [ ] `cross.comm.{agencyID}.reaction.added` published on every successful AddReaction
